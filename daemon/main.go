...
func init() {
	...
	flags.Bool(
		option.AnnounceAutoRouting, true, "Announce direct Linux routes to other cluster nodes via node discovery mechansim")
	viper.BindEnv(option.AnnounceAutoRouting, "CILIUM_AUTO_ROUTING_ANNOUNCE")
...
func initEnv(cmd *cobra.Command) {
	...
	node.ConfigureLocalNode(option.GetRoutingConfiguration())
}
